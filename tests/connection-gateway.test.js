...
(async () => {
  ...
  try {
    ...
    assert.ok(microsoft, 'Microsoft connection state must be merged into the current gateway');
    ...
    assert.ok(liveBody.microsoft, 'current live snapshot must include Microsoft state');
