...
(()=>{
  ...
  async function installLiveMap(){try{
    ...
    searchBox.addListener("places_changed",()=>{const places=searchBox.getPlaces()||[];if(!places.length)return;const place=places[0];if(searchMarker)searchMarker.setMap(null);if(place.geometry?.location){searchMarker=new maps.Marker({map,position:place.geometry.location,title:place.name||place.formatted_address||input.value});if(place.geometry.viewport)map.fitBounds(place.geometry.viewport);else{map.setCenter(place.geometry.location);map.setZoom(17)}const info=new maps.InfoWindow({content:`<div style="color:#111;max-width:260px"><b>${place.name||"Location"}</b><br>${place.formatted_address||input.value}</div>`});info.open({map,anchor:searchMarker});window.dispatchEvent(new CustomEvent("openrabbit:map-search",{detail:{name:place.name||"",address:place.formatted_address||input.value,lat:place.geometry.location.lat(),lng:place.geometry.location.lng(),placeId:place.place_id||""}}))}});
    ...
    locate.onclick=()=>{if(!navigator.geolocation)return;navigator.geolocation.getCurrentPosition(pos=>{const p={lat:pos.coords.latitude,lng:pos.coords.longitude};map.setCenter(p);map.setZoom(15);if(searchMarker)searchMarker.setMap(null);searchMarker=new maps.Marker({map,position:p,title:"Your location"});window.dispatchEvent(new CustomEvent("openrabbit:map-search",{detail:{name:"Your location",address:"",lat:p.lat,lng:p.lng,placeId:""}}))},()=>{})};
    clear.onclick=()=>{input.value="";if(searchMarker){searchMarker.setMap(null);searchMarker=null}map.setCenter({lat:33.4484,lng:-112.0740});map.setZoom(11);window.dispatchEvent(new CustomEvent("openrabbit:map-search",{detail:null}))};
