...
function renderProperty(name, pin){
  ...
  const x = Math.min(pin.offsetLeft + 54, pin.parentElement.clientWidth - 280);
  const y = Math.max(40, pin.offsetTop - 25);
  popover.style.left = `${x}px`;
  popover.style.top = `${y}px`;
  document.getElementById("analyzePropertyBtn")?.addEventListener("click",()=>openAgent(data.prompt));
  document.getElementById("openDealBtn")?.addEventListener("click",()=>{
...
document.querySelectorAll(".pin").forEach((pin)=>pin.addEventListener("click",()=>{
  ...
  renderProperty(pin.dataset.property, pin);
}));
...
runAgent?.addEventListener("click",()=>{
  ...
  const original = runAgent.textContent;
  runAgent.textContent = "Prepared ✓";
  setTimeout(()=>{runAgent.textContent=original; closeAgent();},900);
});

document.querySelectorAll(".saved-search").forEach((item)=>item.addEventListener("click",()=>{
  document.querySelectorAll(".saved-search").forEach((x)=>x.classList.remove("active"));
  item.classList.add("active");
}));

document.querySelectorAll(".tabs, .map-tools").forEach((group)=>group.querySelectorAll("button").forEach((button)=>button.addEventListener("click",()=>{
...
const defaultPin = document.querySelector('.pin[data-property="Royal Palm Inn"]');
if(defaultPin) renderProperty("Royal Palm Inn", defaultPin);
