...
function render(workspace){
  ...
  app.innerHTML=`<div class="grid">
    ...
    <section class="card span4"><div class="label">Workflow state</div><p><strong>${summary.pendingApprovalCount||0}</strong> pending approvals</p><p><strong>${summary.approvedApprovalCount||0}</strong> approved approvals</p>${pending.length?pending.map((x)=>`<div class="approval-row"><span>${escapeHtml(x.taskType)}</span><button class="secondary approveBtn" data-approval-id="${escapeHtml(x.id)}">Approve</button></div>`).join(""):"<p>No pending approval decisions.</p>"}</section>
