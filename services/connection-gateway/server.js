...
const appToken = (process.env.OPENRABBIT_GATEWAY_APP_TOKEN || '').trim();
const encSecret = (process.env.OPENRABBIT_TOKEN_ENCRYPTION_KEY || '').trim();
...
function json(res, status, body, extra = {}) {
  ...
  res.writeHead(status, {
    ...
    'access-control-allow-origin': process.env.OPENRABBIT_GATEWAY_CORS_ORIGIN || '*',
...
function encrypt(obj) {
  ...
  const clear = Buffer.from(JSON.stringify(obj));
...
function userId(req, url) {
  return String(req.headers['x-openrabbit-user'] || url.searchParams.get('user') || 'default')
    .replace(/[^a-zA-Z0-9._@-]/g, '').slice(0, 160) || 'default';
}
function authorized(req) {
  ...
  const a = Buffer.from(supplied), b = Buffer.from(appToken);
  return a.length === b.length && crypto.timingSafeEqual(a, b);
}
function requireAuth(req, res, id) {
  if (authorized(req)) return true;
  json(res, 401, { error: 'UNAUTHORIZED', message: 'OpenRabbit session is not authorized.', requestId: id });
  return false;
}
...
async function connectionState(uid) {
  const results = await Promise.all(providers.map(async provider => {
    const verification = await verifyConnection(uid, provider.id);
    return {
      id: provider.id,
...
async function exchangeGoogle(code, stateRecord) {
  const redirect = `${publicBaseUrl}/oauth/google/callback`;
  const response = await fetch('https://oauth2.googleapis.com/token', {
    method: 'POST', headers: { 'content-type': 'application/x-www-form-urlencoded' },
  ...
  saveToken(stateRecord.uid, stateRecord.id, { ...token, expires_at: Date.now() + Number(token.expires_in || 3600) * 1000 });
}
async function exchangeHubspot(code, stateRecord) {
  const response = await fetch('https://api.hubapi.com/oauth/v1/token', {
    method: 'POST', headers: { 'content-type': 'application/x-www-form-urlencoded' },
  ...
  saveToken(stateRecord.uid, 'hubspot', { ...token, expires_at: Date.now() + Number(token.expires_in || 1800) * 1000 });
}
...
async function route(req, res) {
  ...
  if (req.method === 'GET' && url.pathname === '/health') return json(res, 200, {
    ...
    version: 3,
    configured: { encryption: Boolean(encSecret), auth: Boolean(appToken), google: providerConfigured('gmail'), hubspot: providerConfigured('hubspot'), maps: providerConfigured('google-maps') },
    requestId: id
  });
  ...
  if (url.pathname === '/oauth/google/callback' && req.method === 'GET') {
    ...
    if (url.searchParams.get('error')) return html(res, 400, `<h1>Google authorization failed</h1><p>${url.searchParams.get('error')}</p>`);
    try {
    ...
    } catch (error) { return html(res, 500, `<h1>OpenRabbit connection failed</h1><p>${error.message}</p>`); }
  }
  if (url.pathname === '/oauth/hubspot/callback' && req.method === 'GET') {
    const record = popState(url.searchParams.get('state'));
    if (!record) return html(res, 400, '<h1>OpenRabbit authorization expired</h1>');
    if (url.searchParams.get('error')) return html(res, 400, `<h1>HubSpot authorization failed</h1><p>${url.searchParams.get('error')}</p>`);
    try {
  ...
  if (req.method === 'GET' && url.pathname === '/v1/platform/maps') return json(res, 200, {
    available: providerConfigured('google-maps'),
    browserKey: providerConfigured('google-maps') ? (process.env.GOOGLE_MAPS_BROWSER_KEY || process.env.OPENRABBIT_MAPS_BROWSER_KEY) : '',
    requestId: id
  });
  if (req.method === 'POST' && url.pathname === '/v1/connections/google/start') {
    try { return json(res, 200, { authorizationUrl: googleStart(uid, url.searchParams.get('kind') || 'gmail'), requestId: id }); }
    catch (error) { return json(res, 503, { error: 'PROVIDER_NOT_CONFIGURED', message: error.message, requestId: id }); }
  }
  if (req.method === 'POST' && url.pathname === '/v1/connections/hubspot/start') {
    try { return json(res, 200, { authorizationUrl: hubspotStart(uid), requestId: id }); }
    catch (error) { return json(res, 503, { error: 'PROVIDER_NOT_CONFIGURED', message: error.message, requestId: id }); }
  }
  const verifyMatch = url.pathname.match(/^\/v1\/connections\/([a-z0-9-]+)\/verify$/);
  if (verifyMatch && req.method === 'POST') return json(res, 200, { provider: verifyMatch[1], ...(await verifyConnection(uid, verifyMatch[1])), requestId: id });
  const match = url.pathname.match(/^\/v1\/connections\/([a-z0-9-]+)$/);
  if (match && req.method === 'DELETE') { deleteToken(uid, match[1]); return json(res, 200, { connected: false, provider: match[1], requestId: id }); }
...
if (require.main === module) server.listen(port, host, () => console.log(`OpenRabbit Connection Gateway listening on ${publicBaseUrl}`));
module.exports = { server, providers, connectionState, verifyConnection };
