...
const server=http.createServer(async(req,res)=>{
  try{
    ...
    if(req.method==='GET'&&url.pathname==='/health')return json(res,200,{ok:true,service:'openrabbit-connection-gateway',version:7,configured:{microsoft:ms.configured(),google:Boolean(process.env.GOOGLE_OAUTH_CLIENT_ID&&process.env.GOOGLE_OAUTH_CLIENT_SECRET),hubspot:Boolean(process.env.HUBSPOT_OAUTH_CLIENT_ID&&process.env.HUBSPOT_OAUTH_CLIENT_SECRET),maps:Boolean(process.env.GOOGLE_MAPS_BROWSER_KEY),meta:Boolean(process.env.META_APP_ID&&process.env.META_APP_SECRET),linkedin:Boolean(process.env.LINKEDIN_CLIENT_ID&&process.env.LINKEDIN_CLIENT_SECRET),tiktok:Boolean(process.env.TIKTOK_CLIENT_KEY&&process.env.TIKTOK_CLIENT_SECRET)},actions:{gmailSend:true,googleCalendarCreate:true,hubspotUpdate:true}});
    if(req.method==='POST'&&url.pathname.startsWith('/v1/actions/'))return execute(req,res,url);
  ...
  }catch(e){console.error(e);if(!res.headersSent)json(res,500,{error:'INTERNAL_ERROR',message:e.message||'Connection gateway request failed.'});}
});
if(require.main===module)server.listen(port,host,()=>console.log(`OpenRabbit Connection Gateway v7 listening on ${publicBaseUrl}`));
