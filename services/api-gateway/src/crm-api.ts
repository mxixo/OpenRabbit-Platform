...
function invalid(message: string, code = "INVALID_CRM_REQUEST"): PlatformApiRouteResult { return { matched: true, status: 400, error: { code, message } }; }
...
function isRouteResult(value: unknown): value is PlatformApiRouteResult { return Boolean(value && typeof value === "object" && "matched" in value); }
async function linkRelationship(graph: InMemoryContextGraphStore | undefined, orgId: string, record: { id: string; propertyIds?: string[] }): Promise<void> {
  if (graph) await autoLinkRecordContext(graph, orgId, { type: "relationship", id: record.id }, record);
}
...
export async function routeNativeCrmApi(
  ...
  if (path[0] !== "v1" || path[1] !== "orgs" || !path[2] || path[3] !== "crm") return { matched: false };
  const orgId = path[2];
  ...
  if (path[4] === "import" && path.length === 5 && method === "POST") {
    ...
    try {
      ...
      for (const relationshipId of result.relationshipIds) {
        const record = await store.get(orgId, relationshipId);
        if (record) await linkRelationship(graph, orgId, record);
      }
      return { matched: true, status: 200, data: result };
    } catch (error) {
      return { matched: true, status: 400, error: { code: "CRM_IMPORT_FAILED", message: error instanceof Error ? error.message : "CRM import failed" } };
    }
  }

  if (path[4] !== "relationships") return { matched: false };
  if (method === "GET" && path.length === 5) return { matched: true, status: 200, data: await store.list(orgId) };
  if (method === "POST" && path.length === 5) {
    const parsed = parseCreate(request.body);
    if (isRouteResult(parsed)) return parsed;
    try {
      const record = await store.create(orgId, parsed);
      await linkRelationship(graph, orgId, record);
      return { matched: true, status: 201, data: record };
    } catch (error) {
  ...
  if (path.length === 6) {
    ...
    if (method === "GET") {
      ...
      return record ? { matched: true, status: 200, data: record } : { matched: true, status: 404, error: { code: "CRM_RELATIONSHIP_NOT_FOUND", message: `CRM relationship not found: ${id}` } };
    }
    if (method === "PATCH") {
      const parsed = parseUpdate(request.body);
      if (isRouteResult(parsed)) return parsed;
      try {
        const record = await store.update(orgId, id, parsed);
        await linkRelationship(graph, orgId, record);
        return { matched: true, status: 200, data: record };
      } catch (error) {
    ...
    if (method === "DELETE") {
      ...
      return removed ? { matched: true, status: 200, data: { id, deleted: true } } : { matched: true, status: 404, error: { code: "CRM_RELATIONSHIP_NOT_FOUND", message: `CRM relationship not found: ${id}` } };
    }
  }
