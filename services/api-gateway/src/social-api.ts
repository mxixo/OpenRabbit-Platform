...
function invalid(message: string, code = "INVALID_SOCIAL_REQUEST"): PlatformApiRouteResult { return { matched: true, status: 400, error: { code, message } }; }

export async function routeSocialApi(
  ...
  if (path[4] === "posts") {
    ...
    if (method === "POST" && path.length === 5) {
      const body = (request.body ?? {}) as Partial<{ title: string; network: string; body: string; campaign: string; summary: string; scheduledAt: string; relationshipId: string; propertyId: string; createdBy: "user" | "worker"; status: "draft" | "pending_approval" | "scheduled" }>;
      ...
      const post = await store.create(orgId, body);
      if (graph) await autoLinkRecordContext(graph, orgId, { type: "social", id: post.id, label: post.title }, post);
      return { matched: true, status: 201, data: post };
    }
    if (method === "GET" && path.length === 6) {
      const post = await store.get(orgId, path[5]);
      return post ? { matched: true, status: 200, data: post } : { matched: true, status: 404, error: { code: "SOCIAL_POST_NOT_FOUND", message: "Social post not found" } };
    }
    if (method === "PATCH" && path.length === 6) {
      const body = (request.body ?? {}) as Partial<{ title: string; body: string; campaign: string; scheduledAt: string; status: SocialPostStatus; summary: string }>;
      if (body.status && !STATUSES.has(body.status)) return invalid("invalid social post status", "INVALID_SOCIAL_POST_STATUS");
      try {
        const post = await store.update(orgId, path[5], body);
        if (graph) await autoLinkRecordContext(graph, orgId, { type: "social", id: post.id, label: post.title }, post);
        return { matched: true, status: 200, data: post };
      } catch (error) {
  ...
  if (path[4] === "policy") {
    if (method === "GET" && path.length === 5) return { matched: true, status: 200, data: await store.getPolicy(orgId) };
    if (method === "PUT" && path.length === 5) {
      const body = (request.body ?? {}) as Partial<{ autonomyMode: SocialAutonomyMode; allowedNetworks: string[]; maxPostsPerDay: number; approvalRequiredForNetworks: string[]; quietHours: { start: string; end: string } }>;
