...
function loadGoogleProductivityConfig(env=process.env){return {
  ...
  refreshToken:required(env,'GOOGLE_OAUTH_REFRESH_TOKEN'),
  gmailUserId:env.GOOGLE_GMAIL_USER_ID||'me',
  calendarId:env.GOOGLE_CALENDAR_ID||'primary',
  scanQuery:env.OPENRABBIT_MAIL_SCAN_QUERY||'is:unread',
  scanLimit:numberEnv(env,'OPENRABBIT_MAIL_SCAN_LIMIT',25),
  scanIntervalMs:numberEnv(env,'OPENRABBIT_MAIL_SCAN_INTERVAL_MS',300000),
  proposalFile:env.OPENRABBIT_MEETING_PROPOSAL_FILE||path.join(process.cwd(),'.openrabbit','meeting-proposals.json')
};}
...
function createGoogleProductivityRuntime(config,{extractor,fetchImpl=globalThis.fetch,onScanResult=()=>{},onScanError=()=>{}}={}){
  ...
  const apiClient=new GoogleApiClient({tokenProvider,fetchImpl});
  const mailClient=new GmailClient({apiClient,userId:config.gmailUserId});
  const calendarClient=new GoogleCalendarClient({apiClient,calendarId:config.calendarId});
  const capabilityProvider=new ProductivityCapabilityProviderAdapter({mailClient,calendarClient});
  const detector=new StructuredMeetingDetector({extractor});
  const store=new DurableMeetingProposalStore({filePath:config.proposalFile});
  const workflow=new MeetingProposalWorkflow({capabilityProvider,detector,store});
...
module.exports={loadGoogleProductivityConfig,createGoogleProductivityRuntime};
