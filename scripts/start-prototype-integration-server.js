...
function parseJsonText(text){const raw=String(text||"").trim().replace(/^```json\s*/i,"").replace(/```$/," ").trim();try{return JSON.parse(raw)}catch{const a=raw.indexOf("["),b=raw.lastIndexOf("]");if(a>=0&&b>a){try{return JSON.parse(raw.slice(a,b+1))}catch{}}return[]}}
...
async function scanInboxForMeetings({force=false}={}){
  ...
  const prompt=[
    ...
    "Use ISO-8601 datetimes when the email gives enough evidence. If date or time is ambiguous, set start and end to null; do not guess.",
    "Default duration to 60 minutes only when a start time is explicit but no end time is stated.",
    `Current time: ${new Date().toISOString()}`,
    `Emails: ${JSON.stringify(candidates.map(m=>({messageId:m.id,from:m.from,to:m.to,subject:m.subject,date:m.date,snippet:m.snippet})))}`
  ].join("\n");
  ...
  for(const item of Array.isArray(extracted)?extracted:[]){
    ...
    const source=candidates.find(m=>m.id===item.messageId);
    const proposal={id:crypto.randomUUID(),type:"meeting",status:"pending",source:"gmail",sourceMessageId:item.messageId,sourceSubject:source?.subject||"",sourceFrom:source?.from||"",title:String(item.title||source?.subject||"Proposed meeting"),start:item.start||null,end:item.end||null,location:item.location||"",attendees:Array.isArray(item.attendees)?item.attendees.filter(Boolean):[],confidence:Number(item.confidence||0),rationale:String(item.rationale||"Identified from email"),createdAt:new Date().toISOString()};
...
const server=http.createServer(async(req,res)=>{try{const url=new URL(req.url,APP_ORIGIN);
  ...
  if(req.method==="POST"&&url.pathname==="/api/proactive/scan"){const result=await scanInboxForMeetings({force:Boolean((await readJson(req)).force)});return sendJson(res,200,result)}
