...
const gmail = new GmailOAuthAdapter({ clientId:process.env.GOOGLE_CLIENT_ID, clientSecret:process.env.GOOGLE_CLIENT_SECRET, redirectUri:process.env.GOOGLE_REDIRECT_URI || `${APP_ORIGIN}/api/integrations/gmail/callback`, registry });
...
const agentGateway = new AgentGateway({
  providers: openAIProvider ? { openai:openAIProvider } : {},
  approvalPolicy:{ requiresApproval:(action)=>Boolean(action?.external || action?.consequential) },
  audit:{ record:async(event)=>{ auditEvents.push(event); if(auditEvents.length>100)auditEvents.shift(); } },
});
...
async function readJson(req,{maxBytes=100_000}={}){return new Promise((resolve,reject)=>{let size=0,raw="";req.setEncoding("utf8");req.on("data",chunk=>{size+=Buffer.byteLength(chunk);if(size>maxBytes){reject(new Error("Request body too large"));req.destroy();return}raw+=chunk});req.on("end",()=>{try{resolve(raw?JSON.parse(raw):{})}catch{reject(new Error("Invalid JSON body"))}});req.on("error",reject)})}

async function activeGmailConnection(){let connection=await registry.findProviderConnection({userId:USER_ID,provider:"gmail"});if(!connection)return null;if(connection.expiresAt&&connection.expiresAt<=Date.now()+60_000&&connection.refreshToken){const refreshed=await gmail.refresh(connection.refreshToken);await registry.save({...connection,accessToken:refreshed.access_token,refreshToken:refreshed.refresh_token||connection.refreshToken,expiresAt:Date.now()+Number(refreshed.expires_in||3600)*1000,scopes:connection.scopes});connection=await registry.findProviderConnection({userId:USER_ID,provider:"gmail"})}return connection}

const server=http.createServer(async(req,res)=>{try{
  const url=new URL(req.url,APP_ORIGIN);
  if(req.method==="GET"&&url.pathname==="/api/health")return sendJson(res,200,{ok:true,service:"openrabbit-prototype",gmailConfigured:Boolean(process.env.GOOGLE_CLIENT_ID&&process.env.GOOGLE_CLIENT_SECRET),openaiConfigured:Boolean(openAIProvider),providerToolCount:configuredProviderTools.length});
  if(req.method==="GET"&&url.pathname==="/api/integrations/gmail/connect"){const redirectTo=safeAppPath(url.searchParams.get("redirectTo")||"/communications.html");const{url:authorizationUrl}=gmail.authorizationUrl({userId:USER_ID,redirectTo});return redirect(res,authorizationUrl)}
  if(req.method==="GET"&&url.pathname==="/api/integrations/gmail/callback"){const error=url.searchParams.get("error");if(error)return redirect(res,`/communications.html?gmail=error&reason=${encodeURIComponent(error)}`);const code=url.searchParams.get("code"),state=url.searchParams.get("state");if(!code||!state)return sendJson(res,400,{error:"Missing OAuth code/state"});const result=await gmail.exchangeCode({code,state});const destination=safeAppPath(result.redirectTo);return redirect(res,`${destination}${destination.includes("?")?"&":"?"}gmail=connected`)}
  if(req.method==="GET"&&url.pathname==="/api/integrations/gmail/status"){const connection=await activeGmailConnection();return sendJson(res,200,connection?{connected:true,accountId:connection.accountId,scopes:connection.scopes}:{connected:false})}
  if(req.method==="GET"&&url.pathname==="/api/integrations/gmail/inbox"){const connection=await activeGmailConnection();if(!connection)return sendJson(res,401,{error:"Gmail not connected",connectUrl:"/api/integrations/gmail/connect"});const maxResults=Math.max(1,Math.min(30,Number(url.searchParams.get("limit")||12)));const query=url.searchParams.get("q")||"in:inbox";const inbox=await gmail.listInbox({accessToken:connection.accessToken,maxResults,query});return sendJson(res,200,{connected:true,accountId:connection.accountId,...inbox})}
