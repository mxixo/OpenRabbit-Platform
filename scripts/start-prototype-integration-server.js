...
function safeAppPath(pathname) {
  if (!pathname || !pathname.startsWith("/") || pathname.startsWith("//")) return "/communications.html";
  return pathname;
}

async function activeGmailConnection() {
  const connection = await registry.findProviderConnection({ userId: USER_ID, provider: "gmail" });
  ...
  if (connection.expiresAt && connection.expiresAt <= Date.now() + 60_000 && connection.refreshToken) {
    ...
    return registry.findProviderConnection({ userId: USER_ID, provider: "gmail" });
  }
  return connection;
}
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (req.method === "GET" && url.pathname === "/api/health") {
      return sendJson(res, 200, { ok: true, service: "openrabbit-prototype", gmailConfigured: Boolean(process.env.GOOGLE_CLIENT_ID && process.env.GOOGLE_CLIENT_SECRET) });
    }
...
server.listen(PORT, () => {
  console.log(`OpenRabbit prototype integration server listening on ${APP_ORIGIN}`);
});